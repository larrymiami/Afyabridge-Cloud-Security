...
**Validated:** v0.10A Security gates workflow run #103 (`31182652111`) on implementation commit `0e066b266c1ca98f8133fb369938074f8f1adcee` validated the 18-control posture profile, governance contract, implementation-reference integrity, negative tests, and protected-main verdict integration. v0.10B Security gates run #140 (`31188587045`) on final reviewed implementation/evidence commit `14d70db5040dc0fd22539d07ab50f53de51ce72c` validated 11 executable repository posture rules with 39 assertions and 11 anchored rule/control bindings, a clean 11/11 desired-state evaluation, 10 rule-governance scenarios, 15 collector-compromise scenarios including shortened KMS safety durations, 17 evaluator decision scenarios, 6 Terraform drift-decision scenarios, posture evidence upload, the existing security scanner/policy suite, and the fail-closed `Security gate verdict`. Application baseline run #168 succeeded for the same revision, and Supply chain run #110 completed its unprivileged build/SBOM/evidence path with signing skipped as designed for a pull request. Detailed evidence is recorded in `docs/evidence/v0.10a-validation.md` and `docs/evidence/v0.10b-validation.md`.
